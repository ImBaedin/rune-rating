...
function percentileLabel(score: number) {
  if (score >= 925) return "Top 1% profile";
  if (score >= 875) return "Top 3% profile";
  if (score >= 775) return "Top 10% profile";
  if (score >= 650) return "Top 25% profile";
  if (score >= 525) return "Established profile";
  if (score >= 400) return "Midgame profile";
  return "Progressing profile";
}
...
export const get = query({
  ...
  handler: async (ctx, args) => {
    ...
    const totalLevel =
      ...
      skills.reduce(
        (total, skill) => total + (metricValue(skill.level) ?? 0),
        0,
      );
    const totalXp =
    ...
    const combatTierBonus = clamp(
      combatAchievementsSnapshot.data.points / 1_500,
    );
    const combatScore =
      bossBreadth * 55 +
      bossDepth * 70 +
